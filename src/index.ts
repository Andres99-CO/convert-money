...
const inputCurrency: HTMLInputElement = document.querySelector('#currency') as HTMLInputElement;
...
bAdd!.addEventListener('click', e => {
    if (inputTitle!.value != '' && inputCost!.value != '' && !isNaN(parseFloat(inputCost.value))) {
        ...
        expenses.add({ title, cost: { count: cost, currency: currency } })
        render();
    } else {
...
function render() {
    ...
    expenses.getItems().forEach((item) => {
        ...
        const { count, currency } = cost;
        html += `
            <div class="item">
                <div><span class="currency">${currency} </span>${count} </div>
                <div>${title} </div>
                <div>
                    <button class="bEliminar" data-id="${id}">Eliminar</button>
                </div>
            </div>
        `;
    });
